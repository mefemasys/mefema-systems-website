<section class="section-py landing-services" id="landingServices">
    <div class="container position-relative">
        <div class="row g-4">
            
            <div class="col-lg-6">
                <div class="service-card h-100 d-flex flex-column">
                    <div class="service-header">
                        <div class="service-icon">
                            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M3 9H21M9 21V9M12 3H4C2.89543 3 2 3.89543 2 5V19C2 20.1046 2.89543 21 4 21H20C21.1046 21 22 20.1046 22 19V9M12 3L14 5M12 3L10 5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </div>
                        <h4 class="service-title">Websites Prontos</h4>
                    </div>
                    <ul class="service-list flex-grow-1">
                        <li><span class="check-icon">✓</span> Domínio (.co.mz) e hospedagem incluídos</li>
                        <li><span class="check-icon">✓</span> SSL e E-mail profissional ilimitado</li>
                        <li><span class="check-icon">✓</span> Design moderno, responsivo e ultra-rápido</li>
                        <li><span class="check-icon">✓</span> Otimização SEO para buscas no Google</li>
                        <li><span class="check-icon">✓</span> Painel administrativo para gestão de conteúdo</li>
                        <li><span class="check-icon">✓</span> Integração com WhatsApp e Redes Sociais</li>
                    </ul>
                    <div class="mt-4">
                        <p class="fw-bold mb-2">A partir de 25.000 MT</p>
                        <a href="/produtos/websites" class="service-btn w-100 d-flex justify-content-between align-items-center">
                            <span>Ver todos os pacotes</span>
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                                <path d="M5 12H19M19 12L12 5M19 12L12 19" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </a>
                    </div>
                </div>
            </div>

            <div class="col-lg-6">
                <div class="service-card h-100 d-flex flex-column">
                    <div class="service-header">
                        <div class="service-icon">
                            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M20 7H4C2.89543 7 2 7.89543 2 9V19C2 20.1046 2.89543 21 4 21H20C21.1046 21 22 20.1046 22 19V9C22 7.89543 21.1046 7 20 7Z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/>
                                <path d="M16 21V5C16 4.46957 15.7893 3.96086 15.4142 3.58579C15.0391 3.21071 14.5304 3 14 3H10C9.46957 3 8.96086 3.21071 8.58579 3.58579C8.21071 3.96086 8 4.46957 8 5V21" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/>
                            </svg>
                        </div>
                        <h4 class="service-title">Sistemas de Gestão</h4>
                    </div>
                    <ul class="service-list flex-grow-1">
                        <li><span class="check-icon">✓</span> Certificação AT (quando aplicável)</li>
                        <li><span class="check-icon">✓</span> Facturação, Stock, CRM e RH integrados</li>
                        <li><span class="check-icon">✓</span> Multi-utilizador com níveis de acesso</li>
                        <li><span class="check-icon">✓</span> Relatórios avançados e Dashboards</li>
                        <li><span class="check-icon">✓</span> Integração M-Pesa, e-Mola e Bancos</li>
                        <li><span class="check-icon">✓</span> Backups diários automáticos na nuvem</li>
                    </ul>
                    <div class="mt-4">
                        <p class="fw-bold mb-2">A partir de 45.000 MT</p>
                        <a href="/produtos/sistemas-gestao" class="service-btn w-100 d-flex justify-content-between align-items-center">
                            <span>Explorar sistemas</span>
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                                <path d="M5 12H19M19 12L12 5M19 12L12 19" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </a>
                    </div>
                </div>
            </div>

            <div class="col-lg-6">
                <div class="service-card h-100 d-flex flex-column">
                    <div class="service-header">
                        <div class="service-icon">
                            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect x="5" y="2" width="14" height="20" rx="3" stroke="currentColor" stroke-width="2"/>
                                <path d="M12 18H12.01" stroke="currentColor" stroke-width="3" stroke-linecap="round"/>
                            </svg>
                        </div>
                        <h4 class="service-title">Apps Móveis</h4>
                    </div>
                    <ul class="service-list flex-grow-1">
                        <li><span class="check-icon">✓</span> Publicação nas lojas (Play e App Store)</li>
                        <li><span class="check-icon">✓</span> Notificações Push e Geolocalização</li>
                        <li><span class="check-icon">✓</span> Funcionamento Offline e Sincronização</li>
                        <li><span class="check-icon">✓</span> Integração total com pagamentos móveis</li>
                        <li><span class="check-icon">✓</span> Interface UI/UX moderna e intuitiva</li>
                        <li><span class="check-icon">✓</span> Suporte a Delivery, Táxi e E-commerce</li>
                    </ul>
                    <div class="mt-4">
                        <p class="fw-bold mb-2">A partir de 80.000 MT</p>
                        <a href="/produtos/mobile-apps" class="service-btn w-100 d-flex justify-content-between align-items-center">
                            <span>Ver aplicações</span>
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                                <path d="M5 12H19M19 12L12 5M19 12L12 19" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </a>
                    </div>
                </div>
            </div>

            <div class="col-lg-6">
                <div class="service-card h-100 d-flex flex-column">
                    <div class="service-header">
                        <div class="service-icon">
                            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12 16V22M12 16C14.2091 16 16 14.2091 16 12C16 9.79086 14.2091 8 12 8C9.79086 8 8 9.79086 8 12C8 14.2091 9.79086 16 12 16ZM12 8V2M12 8L18 5M12 8L6 5M12 16L18 19M12 16L6 19" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                            </svg>
                        </div>
                        <h4 class="service-title">Soluções IA + IoT</h4>
                    </div>
                    <ul class="service-list flex-grow-1">
                        <li><span class="check-icon">✓</span> Chatbots avançados (WhatsApp e Site)</li>
                        <li><span class="check-icon">✓</span> Automação e Monitorização remota</li>
                        <li><span class="check-icon">✓</span> Dashboards preditivos com análise de dados</li>
                        <li><span class="check-icon">✓</span> Integração com sensores e hardware</li>
                        <li><span class="check-icon">✓</span> Inteligência Artificial para tomada de decisão</li>
                        <li><span class="check-icon">✓</span> Reconhecimento facial e visão computacional</li>
                    </ul>
                    <div class="mt-4">
                        <p class="fw-bold mb-2">Preço: Sob consulta</p>
                        <a href="/produtos/iot-ia" class="service-btn w-100 d-flex justify-content-between align-items-center">
                            <span>Saber mais</span>
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                                <path d="M5 12H19M19 12L12 5M19 12L12 19" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </a>
                    </div>
                </div>
            </div>

        </div>
    </div>
</section>
<style>
/* ========================================
   VARIÁVEIS DOS SERVIÇOS
   ======================================== */

:root {
    --services-primary: #d97638;
    --services-primary-light: #ff8c4a;
    --services-secondary: #c66b3d;
    --services-accent: #b85f30;
    
    --services-bg: #fdf8f4;
    --services-card-bg: #fffdfb;
    --services-text: #5a3a28;
    --services-text-secondary: #74523d;
    --services-text-muted: #8a6850;
    --services-border: rgba(217, 118, 56, 0.35);
    
    --services-shadow: rgba(217, 118, 56, 0.08);
    --services-shadow-hover: rgba(217, 118, 56, 0.2);
}

@media (prefers-color-scheme: dark) {
    :root {
        --services-primary: #ff8c4a;
        --services-primary-light: #ffa366;
        --services-secondary: #d97638;
        --services-accent: #c66b3d;
        
        --services-bg: #12100d;
        --services-card-bg: #1f1814;
        --services-text: #f5ede6;
        --services-text-secondary: #d4c5b8;
        --services-text-muted: #9a8d82;
        --services-border: rgba(255, 140, 74, 0.15);
        
        --services-shadow: rgba(0, 0, 0, 0.3);
        --services-shadow-hover: rgba(255, 140, 74, 0.25);
    }
}

[data-bs-theme="light"] {
    --services-bg: #fdf8f4;
    --services-card-bg: #fffdfb;
    --services-text: #5a3a28;
    --services-text-secondary: #74523d;
    --services-text-muted: #8a6850;
    --services-border: rgba(217, 118, 56, 0.35);
}

[data-bs-theme="dark"] {
    --services-bg: #12100d;
    --services-card-bg: #1f1814;
}

/* ========================================
   SECÇÃO PRINCIPAL
   ======================================== */

.landing-services {
    background: var(--services-bg);
    position: relative;
}

/* ========================================
   CABEÇALHO
   ======================================== */

.services-badge {
    display: inline-flex;
    align-items: center;
    gap: 0.75rem;
    padding: 0.75rem 1.75rem;
    background: linear-gradient(135deg, rgba(217, 118, 56, 0.12), rgba(184, 95, 48, 0.08));
    border: 1px solid var(--services-border);
    border-radius: 5px;
    backdrop-filter: blur(10px);
}

.badge-icon {
    color: var(--services-primary);
    font-size: 1rem;
    animation: badgePulse 2s ease-in-out infinite;
}

.badge-text {
    color: var(--services-primary);
    font-weight: 600;
    font-size: 0.875rem;
    letter-spacing: 1.5px;
}

.services-title {
    font-size: 2.5rem;
    font-weight: 700;
    color: var(--services-text);
    line-height: 1.2;
}

.text-gradient {
    background: linear-gradient(135deg, var(--services-primary), var(--services-secondary));
    -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;
    background-clip: text;
}

.services-subtitle {
    max-width: 650px;
    font-size: 1.1rem;
    color: var(--services-text-muted);
    line-height: 1.7;
}

/* ========================================
   CARDS DE SERVIÇOS
   ======================================== */

.service-card {
    background: var(--services-card-bg);
    border-radius: 6px;
    padding: 2.5rem;
    height: 100%;
    border: 1px solid var(--services-border);
    box-shadow: 0 4px 20px var(--services-shadow);
    transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1);
    position: relative;
    overflow: hidden;
    display: flex;
    flex-direction: column;
}

.service-card::before {
    content: '';
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 4px;
    background: linear-gradient(90deg, var(--services-primary), var(--services-primary-light));
    transform: scaleX(0);
    transform-origin: left;
    transition: transform 0.4s ease;
}

.service-card:hover {
    transform: translateY(-8px);
    box-shadow: 0 12px 40px var(--services-shadow-hover);
    border-color: var(--services-primary);
}

.service-card:hover::before {
    transform: scaleX(1);
}

/* ========================================
   CABEÇALHO DO CARD
   ======================================== */

.service-header {
    display: flex;
    align-items: center;
    gap: 1.25rem;
    margin-bottom: 2rem;
    padding-bottom: 1.5rem;
    border-bottom: 1px solid var(--services-border);
}

.service-icon {
    width: 64px;
    height: 64px;
    min-width: 64px;
    display: flex;
    align-items: center;
    justify-content: center;
    background: linear-gradient(135deg, rgba(217, 118, 56, 0.15), rgba(217, 118, 56, 0.05));
    border-radius: 4px;
    color: var(--services-primary);
    transition: all 0.4s ease;
}

.service-card:hover .service-icon {
    transform: scale(1.1) rotate(5deg);
    background: linear-gradient(135deg, rgba(217, 118, 56, 0.25), rgba(217, 118, 56, 0.1));
}

.service-title {
    font-size: 1.5rem;
    font-weight: 600;
    color: var(--services-text);
    margin: 0;
    transition: color 0.3s ease;
}

.service-card:hover .service-title {
    color: var(--services-primary);
}

/* ========================================
   LISTA DE SERVIÇOS
   ======================================== */

.service-list {
    list-style: none;
    padding: 0;
    margin: 0 0 2rem 0;
    flex-grow: 1;
}

.service-list li {
    display: flex;
    align-items: flex-start;
    gap: 1rem;
    padding: 0.75rem 0;
    color: var(--services-text-secondary);
    font-size: 0.95rem;
    line-height: 1.6;
    transition: all 0.3s ease;
}

.service-list li:hover {
    color: var(--services-text);
    transform: translateX(5px);
}

.check-icon {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    width: 24px;
    height: 24px;
    min-width: 24px;
    background: linear-gradient(135deg, var(--services-primary), var(--services-secondary));
    color: white;
    border-radius: 5%;
    font-size: 0.75rem;
    font-weight: bold;
    transition: transform 0.3s ease;
}

.service-list li:hover .check-icon {
    transform: scale(1.15);
}

/* ========================================
   BOTÃO DO CARD
   ======================================== */

.service-btn {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    gap: 0.75rem;
    padding: 1rem 2rem;
    background: transparent;
    border: 2px solid var(--services-primary);
    border-radius: 3px;
    color: var(--services-primary);
    font-weight: 600;
    text-decoration: none;
    transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1);
    position: relative;
    overflow: hidden;
}

.service-btn::before {
    content: '';
    position: absolute;
    top: 0;
    left: 0;
    width: 0;
    height: 100%;
    background: var(--services-primary);
    transition: width 0.4s cubic-bezier(0.4, 0, 0.2, 1);
    z-index: -1;
}

.service-btn:hover {
    color: white;
    transform: translateX(5px);
}

.service-btn:hover::before {
    width: 100%;
}

.service-btn svg {
    transition: transform 0.3s ease;
}

.service-btn:hover svg {
    transform: translateX(5px);
}

/* ========================================
   ANIMAÇÕES
   ======================================== */

@keyframes badgePulse {
    0%, 100% {
        transform: scale(1);
    }
    50% {
        transform: scale(1.05);
    }
}

/* ========================================
   RESPONSIVIDADE
   ======================================== */

@media (max-width: 1199px) {
    .services-title {
        font-size: 2rem;
    }
    
    .service-card {
        padding: 2rem;
    }
    
    .service-icon {
        width: 56px;
        height: 56px;
        min-width: 56px;
    }
    
    .service-title {
        font-size: 1.35rem;
    }
}

@media (max-width: 991px) {
    .service-header {
        flex-direction: column;
        align-items: flex-start;
        gap: 1rem;
    }
    
    .service-icon {
        width: 48px;
        height: 48px;
        min-width: 48px;
    }
}

@media (max-width: 767px) {
    .services-title {
        font-size: 1.75rem;
    }
    
    .services-subtitle {
        font-size: 1rem;
    }
    
    .service-card {
        padding: 1.75rem;
    }
    
    .service-title {
        font-size: 1.25rem;
    }
    
    .service-list li {
        font-size: 0.9rem;
    }
    
    .service-btn {
        width: 100%;
        padding: 0.875rem 1.5rem;
    }
}

@media (max-width: 575px) {
    .badge-text {
        font-size: 0.75rem;
    }
    
    .services-title {
        font-size: 1.5rem;
    }
    
    .services-subtitle {
        font-size: 0.95rem;
    }
    
    .service-card {
        padding: 1.5rem;
    }
}

/* ========================================
   MODO DARK - AJUSTES ESPECÍFICOS
   ======================================== */

@media (prefers-color-scheme: dark) {
    .service-card {
        box-shadow: 0 4px 20px var(--services-shadow);
    }
    
    .service-card:hover {
        box-shadow: 0 12px 40px var(--services-shadow-hover);
    }
    
    .service-icon {
        background: linear-gradient(135deg, rgba(255, 140, 74, 0.2), rgba(255, 140, 74, 0.08));
    }
    
    .service-card:hover .service-icon {
        background: linear-gradient(135deg, rgba(255, 140, 74, 0.3), rgba(255, 140, 74, 0.12));
    }
}
</style>
