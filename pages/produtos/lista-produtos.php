...
/* Botão de demo */
.btn-demo { background-color: #6c757d; border-color: #6c757d; color: white; }
.btn-demo:hover { background-color: #5a6268; border-color: #5a6268; color: white; }
</style>
...
<script>
// Dados dos produtos (preenchidos hardcoded por enquanto, mas preparados para API)
let produtos = {
  websites: [
    {
      nome: 'Website Institucional',
      descricao: 'Website profissional, optimizado, rápido e responsivo. Ideal para apresentar sua empresa de forma elegante e funcional, com foco em usabilidade e performance em todos os dispositivos.',
      icone: 'ri-global-line',
      cor: 'orange',
      caracteristicas: ['Até 10 páginas', 'SEO incluído', 'Administração fácil'],
      imagens: ['assets/img/screenshots/website-institucional-1.png', 'assets/img/screenshots/website-institucional-2.png', 'assets/img/screenshots/website-institucional-3.png'], // Slides de screenshots
      demo_url: 'https://demo.website-institucional.example.com' // Botão de demo
    },
    {
      nome: 'Landing Page',
      descricao: 'Página focada em conversão de clientes. Projetada para capturar leads e impulsionar vendas com elementos persuasivos e chamadas para ação claras.',
      icone: 'ri-rocket-line',
      cor: 'blue',
      caracteristicas: ['Alta conversão', 'WhatsApp integrado', 'Pixel Facebook & Google'],
      imagens: ['assets/img/screenshots/landing-page-1.png', 'assets/img/screenshots/landing-page-2.png'], // Slides de screenshots
      demo_url: 'https://demo.landing-page.example.com' // Botão de demo
    },
    {
      nome: 'E-commerce',
      descricao: 'Loja online completa e segura. Plataforma robusta para gerenciar vendas online, com suporte a múltiplos métodos de pagamento e integração logística.',
      icone: 'ri-shopping-cart-line',
      cor: 'green',
      caracteristicas: ['Pagamentos online', 'Gestão de stock', 'Envios automatizados'],
      imagens: ['assets/img/screenshots/ecommerce-1.png', 'assets/img/screenshots/ecommerce-2.png'], // Slides de screenshots
      demo_url: 'https://demo.ecommerce.example.com' // Botão de demo
    },
    {
      nome: 'Blog Profissional',
      descricao: 'Plataforma de conteúdo optimizada. Ferramenta poderosa para criar e gerenciar conteúdo, com recursos para engajamento de audiência e otimização para motores de busca.',
      icone: 'ri-article-line',
      cor: 'brown',
      caracteristicas: ['Editor avançado', 'Multi-autor', 'SEO automático'],
      imagens: ['assets/img/screenshots/blog-1.png'], // Slides de screenshots
      demo_url: 'https://demo.blog.example.com' // Botão de demo
    }
  ],
  sistemas: [
    {
      nome: 'Sistema de Facturação',
      descricao: 'Software certificado pela AT para Moçambique. Solução completa para emissão de faturas, controle de estoque e integrações financeiras locais.',
      icone: 'ri-file-list-3-line',
      cor: 'orange',
      caracteristicas: ['PDV', 'Stock multi-loja', 'M-Pesa integrado'],
      imagens: ['assets/img/screenshots/facturacao-1.png', 'assets/img/screenshots/facturacao-2.png'], // Slides de screenshots
      demo_url: 'https://demo.facturacao.example.com' // Botão de demo
    },
    {
      nome: 'CRM Empresarial',
      descricao: 'Gestão completa de clientes e vendas. Sistema para rastrear interações com clientes, gerenciar funis de vendas e gerar insights acionáveis.',
      icone: 'ri-customer-service-2-line',
      cor: 'blue',
      caracteristicas: ['Pipeline de vendas', 'Automação', 'Relatórios'],
      imagens: ['assets/img/screenshots/crm-1.png'], // Slides de screenshots
      demo_url: 'https://demo.crm.example.com' // Botão de demo
    },
    {
      nome: 'Sistema de RH',
      descricao: 'Gestão de recursos humanos simplificada. Ferramenta para manejar folha de pagamento, benefícios e desenvolvimento de equipe de forma eficiente.',
      icone: 'ri-team-line',
      cor: 'green',
      caracteristicas: ['Folha de pagamento', 'Férias', 'Avaliações'],
      imagens: ['assets/img/screenshots/rh-1.png', 'assets/img/screenshots/rh-2.png'], // Slides de screenshots
      demo_url: 'https://demo.rh.example.com' // Botão de demo
    }
  ],
  outros: [
    {
      nome: 'ERP Simplificado',
      descricao: 'Plataforma completa para PMEs. Integração de processos empresariais em um único sistema para otimizar operações e reduzir custos.',
      icone: 'ri-dashboard-3-line',
      cor: 'brown',
      caracteristicas: ['Financeiro', 'Vendas', 'Stock'],
      imagens: ['assets/img/screenshots/erp-1.png', 'assets/img/screenshots/erp-2.png'], // Slides de screenshots
      demo_url: 'https://demo.erp.example.com' // Botão de demo
    },
    {
      nome: 'App Mobile',
      descricao: 'Aplicação nativa para Android e iOS. Desenvolvimento de apps mobile com foco em performance, usabilidade e funcionalidades avançadas.',
      icone: 'ri-smartphone-line',
      cor: 'orange',
      caracteristicas: ['Design nativo', 'Offline first', 'Push notifications'],
      imagens: ['assets/img/screenshots/app-mobile-1.png'], // Slides de screenshots
      demo_url: 'https://demo.app-mobile.example.com' // Botão de demo
    }
  ]
};

// Função para buscar dados da API (quando existir)
async function fetchProdutos() {
  try {
    const response = await fetch('/api/produtos'); // Endpoint da API (ainda não existe)
    if (!response.ok) {
      throw new Error('Erro ao buscar produtos');
    }
    const data = await response.json();
    produtos = data; // Atualiza os dados com o retorno da API
    // Re-renderiza tudo após fetch
    mudarCategoria('websites');
    mudarCategoria('sistemas');
    mudarCategoria('outros');
  } catch (error) {
    console.error('Falha ao carregar da API, usando dados locais:', error);
    // Usa os dados hardcoded como fallback
  }
}
...
function gerarCard(produto, index, categoria) {
  ...
  const slides = produto.imagens.map((img, i) => `
    <div class="carousel-item ${i === 0 ? 'active' : ''}">
      <img src="${img}" alt="Screenshot de ${produto.nome}" onclick="abrirPreview('${img}')">
    </div>
  `).join('');
  ...
  ` : '';
  const botaoDemo = produto.demo_url ? `
    <a href="${produto.demo_url}" class="btn btn-demo btn-sm px-4 me-2" target="_blank">Demo</a>
  ` : '';
  return `
...
document.addEventListener('DOMContentLoaded', async function() {
  await fetchProdutos(); // Tenta carregar da API, cai no fallback se não existir
  mudarCategoria('websites');
  mudarCategoria('sistemas');
  mudarCategoria('outros');
});
